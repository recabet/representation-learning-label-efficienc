...
def run_experiment(
    ...
    fit_cls(scratch_model, train_loader, device=device, epochs=epochs,
            lr=1e-3, checkpoint_dir=f"checkpoints/scratch/{experiment_name}")
    ...
    fit_cls(imagenet_model, train_loader, device=device, epochs=epochs,
            lr=1e-4, checkpoint_dir=f"checkpoints/imagenet/{experiment_name}")
    ...
    fit_cls(model_probe, train_loader, device=device, epochs=200,
            lr=0.1, checkpoint_dir=f"checkpoints/linear_probe/{experiment_name}",
            use_sgd=True)
    ...
    # ── Save everything ──────────────────────────────────────────────────────
    save_dir = Path("plots") / experiment_name
